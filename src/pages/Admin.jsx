...
function Admin() {
    ...
    useEffect(() => {
        if (table && action === "insert") {
            fetchTableColumns();
            setTableColumnsInsertValue([]); // Limpa os valores ao trocar tabela/ação
        }
    }, [table, action]);
            ...
            <div className="flex bg-black h-screen">
                ...
                {(action === "select" && table) && (
                    <div className="w-full p-12 overflow-hidden">
                        <div className="p-12 bg-zinc-800 h-full w-full rounded-2xl overflow-hidden flex flex-col">
                            <span className="text-white font-semibold text-4xl capitalize">{action} in Table {table}</span>
                            
                            {/* 5. Wrapper para o botão e a tabela */}
                            <div className="relative w-full mt-8 overflow-auto flex-1 flex flex-col"> 
                        ...
                        <form>
                            <div className="flex flex-wrap gap-4 p-20">
                                {tableColumns.map((column) => {
                                    ...
                                    return (
                                        <div key={column} className="mb-4 min-w-[220px] flex-1">
                                            ...
                                            <div className="relative w-full">
                                                <Tag className="absolute left-4 top-1/2 transform -translate-y-1/2 text-gray-400 w-5 h-5 pointer-events-none z-10" />
                                                <input
                                                    {...(column === "id" ? { type: "number" } : { type: "text" })}
                                                    placeholder={column}
